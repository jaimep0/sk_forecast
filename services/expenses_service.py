...
EXPECTED_COLUMNS = ["date", "concept", "total"]
...
def prepare_expenses_dataframe(uploaded_file) -> pd.DataFrame:
    ...
    df["total"] = pd.to_numeric(df["total"].fillna(0), errors="raise").round(2)
    return df
    ...
    try:
        for _, row in df.iterrows():
            existing_row = (
                ...
                .filter(Expenses.date == row["date"], Expenses.concept == row["concept"])
                .first()
            )
            ...
            if existing_row:
                ...
                session.add(Expenses(date=row["date"], concept=row["concept"], total=row["total"]))
                inserted += 1
...
def get_expenses_history() -> pd.DataFrame:
    df = pd.read_sql(select(Expenses), engine)
    if df.empty:
        return df
